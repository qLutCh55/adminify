    ...
    <auth-container>
        <v-card width="320">
            <v-card-text>
                ...
                <v-form @submit.native.prevent="doPasswordChange" ref="resetPasswordForm" class="text-xs-center"
                        lazy-validation>
                    <v-text-field
                            v-model="user.token"
                            class="hidden"
        ...
        methods: {
            doPasswordChange() {
                if (this.$refs.resetPasswordForm.validate()) {
                    ...
                    window.axios.post('/password/reset', this.user, {errorHandle: true}).then(response => {
